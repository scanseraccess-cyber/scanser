/**
    ...
    const translations = {
      en: {
        ...
        submitProcessing: "Uploading & scanning…",
        submitReading: "Reading document…",
        scanStatus1: "Connecting to Turnitin engine…",
        scanStatus2: "Turnitin is analyzing your document…",
        scanStatus3: "Cross-checking against source database…",
        scanStatus4: "Compiling your report…",
        ...
        uploadResultTitle: "Result",
        uploadResultEmptyTitle: "View and download the report.",
        uploadResultEmptyHint: "Process a document to see the score here — you can view or download it once it's ready.",
        uploadResultScoreLabel: "Overall score",
        ...
        testingUnlockedLabel: "Unlimited — testing mode (no recharge needed)",
        ...
        demoStatusChipText: "Rewriting tools are in demo mode — real API keys needed to go live",
        demoStatusReadMore: "Read More",
        wechatStatusChipText: "WeChat Auth not connected — data resets on refresh, no reports yet",
        wechatInfoTitle: "WeChat Auth isn't connected yet 👋",
        wechatInfoIntro: "There's no WeChat Auth wired up on the backend yet, which means there's no database storing anything right now — a page refresh clears your data, and full reports won't show until WeChat Auth is connected. Here's exactly what that means:",
        wechatInfoWorksTitle: "Working right now",
        wechatInfoWorksItems: "Each scan returns a similarity % or AI-generated % score from Turnitin|Results only live in this browser tab/session — nothing is saved anywhere yet|Refreshing or closing the page clears all scan data|Every account currently shares the same demo session — no personal accounts yet",
        wechatInfoPendingTitle: "Once WeChat Auth is connected",
        wechatInfoPendingItems: "Every scan is saved to a real database — nothing lost on refresh|Full, detailed reports (not just the % score) will generate and be viewable, correctly and reliably from Turnitin|Every person gets their own personal account tied to their WeChat login|Scan history, credits and reports all persist properly per account",
        wechatInfoCta: "Got it",
        ...
        noApiConfiguredToast: "No API configured for this tool yet — file upload is disabled until a real provider is connected.",
        ...
        matchedSourcesLabel: "Matched Sources",
        unknownSource: "Unknown source",
        gotIt: "Got it",
        addUserUnavailableMsg: "isn't available from here anymore — real accounts are created by the person signing up themselves from the login screen. Once they've signed up, find them in this list to set their role and tool access.",
        ...
        reportsDeletedToast: "Selected reports deleted",
      ...
      zh: {
        ...
        submitProcessing: "正在上传并检测…",
        scanStatus1: "正在连接 Turnitin 引擎…",
        scanStatus2: "Turnitin 正在分析您的文档…",
        scanStatus3: "正在与来源数据库比对…",
        scanStatus4: "正在生成报告…",
        submitReading: "正在读取文档…",
        scanHistory: "历史检测记录",
        detectionModelLabel: "检测模型",
        chooseModelLabel: "选择检测模型",
        excludeReferencesLabel: "排除引用/参考文献",
        excludeReferencesHint: "评分时将跳过引用内容和参考文献列表。",
        uploadResultTitle: "检测结果",
        uploadResultEmptyTitle: "查看并下载报告。",
        uploadResultEmptyHint: "处理文档后，分数会显示在这里——完成后即可查看或下载。",
        uploadResultScoreLabel: "总分",
        ...
        testingUnlockedLabel: "无限次数 — 测试模式（无需充值）",
        ...
        demoStatusChipText: "改写工具仍为演示模式 · 需接入真实 API 密钥后上线",
        demoStatusReadMore: "查看详情",
        wechatStatusChipText: "微信登录尚未接入 · 刷新后数据会丢失，报告暂不可用",
        wechatInfoTitle: "微信登录尚未接入 👋",
        wechatInfoIntro: "后端目前还没有接入微信登录，所以暂时没有数据库来保存您的数据 — 刷新页面会清空所有数据，完整报告也要等微信登录接入后才会显示。具体情况如下：",
        wechatInfoWorksTitle: "目前可用",
        wechatInfoWorksItems: "每次扫描会返回来自 Turnitin 的相似度百分比或 AI 生成率百分比|结果仅保存在当前浏览器标签页/会话中，尚未保存到任何地方|刷新或关闭页面会清空所有扫描数据|目前所有账号共用同一个演示会话，尚无个人账号",
        wechatInfoPendingTitle: "微信登录接入后",
        wechatInfoPendingItems: "每次扫描都会保存到真实数据库，刷新不会丢失|完整详细的报告（不只是百分比）将由 Turnitin 正确、稳定地生成并可查看|每个人都会拥有绑定其微信账号的专属个人账号|扫描历史、积分与报告都会按账号正确保存",
        wechatInfoCta: "知道了",
        ...
        noApiConfiguredToast: "该工具尚未配置 API，在连接真实服务商之前无法上传文件。",
        docQueued: "文档已提交处理",
        ...
        matchesFound: "个匹配项",
        matchedSourcesLabel: "匹配来源",
        unknownSource: "未知来源",
        gotIt: "知道了",
        addUserUnavailableMsg: "现在无法从此处添加——真实账号由用户本人在登录页面自行注册。注册完成后，可在此列表中找到该用户以设置其角色与工具权限。",
        processingMode: "处理模式",
        modeStandard: "标准模式",
        modeAggressive: "深度模式",
        modeAcademic: "学术模式",
    ...
    // TESTING SWITCH — when true, every tool + every premium rewrite
    // control (platform/strength/mode) is unlocked for every user with no
    // credits spent and no Enhanced Membership required. Flip back to
    // `false` to restore normal credit/membership gating before going live.
    ...
    // Not persisted — resets to sensible defaults each session, same
    // pattern as rewriteUiState above.
    ...
    const PLATFORMS = [
      ...
      { id: 'ithenticate', zh: 'iThenticate', en: 'iThenticate' },
    ...
    // Supported languages for the document/text being scanned or rewritten
    // — shown on the upload tools (See AI Report / See Plagiarism Report)
    // and on the Start Rewriting text-input language selector. Chinese and
    // English stay first since they're the primary/original two, the rest
    // are additional supported languages.
    const LANGUAGES = [
      { id: "zh", en: "Chinese (Simplified)", zh: "中文（简体）" },
      { id: "zh-hant", en: "Chinese (Traditional)", zh: "中文（繁體）" },
      { id: "yue", en: "Chinese (Cantonese)", zh: "粤语" },
      { id: "en", en: "English", zh: "英语" },
      { id: "en-us", en: "English (US)", zh: "英语（美国）" },
      { id: "en-gb", en: "English (UK)", zh: "英语（英国）" },
      { id: "es", en: "Spanish", zh: "西班牙语" },
      { id: "es-mx", en: "Spanish (Latin America)", zh: "西班牙语（拉丁美洲）" },
      { id: "fr", en: "French", zh: "法语" },
      { id: "fr-ca", en: "French (Canada)", zh: "法语（加拿大）" },
      { id: "de", en: "German", zh: "德语" },
      { id: "pt", en: "Portuguese", zh: "葡萄牙语" },
      { id: "pt-br", en: "Portuguese (Brazil)", zh: "葡萄牙语（巴西）" },
      { id: "ru", en: "Russian", zh: "俄语" },
      { id: "ar", en: "Arabic", zh: "阿拉伯语" },
      { id: "ur", en: "Urdu", zh: "乌尔都语" },
      { id: "hi", en: "Hindi", zh: "印地语" },
      { id: "ja", en: "Japanese", zh: "日语" },
      { id: "ko", en: "Korean", zh: "韩语" },
      { id: "it", en: "Italian", zh: "意大利语" },
      { id: "nl", en: "Dutch", zh: "荷兰语" },
      { id: "sv", en: "Swedish", zh: "瑞典语" },
      { id: "no", en: "Norwegian", zh: "挪威语" },
      { id: "nb", en: "Norwegian Bokmål", zh: "书面挪威语" },
      { id: "nn", en: "Norwegian Nynorsk", zh: "新挪威语" },
      { id: "da", en: "Danish", zh: "丹麦语" },
      { id: "fi", en: "Finnish", zh: "芬兰语" },
      { id: "pl", en: "Polish", zh: "波兰语" },
      { id: "cs", en: "Czech", zh: "捷克语" },
      { id: "sk", en: "Slovak", zh: "斯洛伐克语" },
      { id: "sl", en: "Slovenian", zh: "斯洛文尼亚语" },
      { id: "hu", en: "Hungarian", zh: "匈牙利语" },
      { id: "ro", en: "Romanian", zh: "罗马尼亚语" },
      { id: "bg", en: "Bulgarian", zh: "保加利亚语" },
      { id: "el", en: "Greek", zh: "希腊语" },
      { id: "tr", en: "Turkish", zh: "土耳其语" },
      { id: "he", en: "Hebrew", zh: "希伯来语" },
      { id: "th", en: "Thai", zh: "泰语" },
      { id: "vi", en: "Vietnamese", zh: "越南语" },
      { id: "id", en: "Indonesian", zh: "印度尼西亚语" },
      { id: "ms", en: "Malay", zh: "马来语" },
      { id: "fil", en: "Filipino", zh: "菲律宾语" },
      { id: "tl", en: "Tagalog", zh: "他加禄语" },
      { id: "sw", en: "Swahili", zh: "斯瓦希里语" },
      { id: "am", en: "Amharic", zh: "阿姆哈拉语" },
      { id: "uk", en: "Ukrainian", zh: "乌克兰语" },
      { id: "be", en: "Belarusian", zh: "白俄罗斯语" },
      { id: "lt", en: "Lithuanian", zh: "立陶宛语" },
      { id: "lv", en: "Latvian", zh: "拉脱维亚语" },
      { id: "et", en: "Estonian", zh: "爱沙尼亚语" },
      { id: "hr", en: "Croatian", zh: "克罗地亚语" },
      { id: "sr", en: "Serbian", zh: "塞尔维亚语" },
      { id: "bs", en: "Bosnian", zh: "波斯尼亚语" },
      { id: "mk", en: "Macedonian", zh: "马其顿语" },
      { id: "sq", en: "Albanian", zh: "阿尔巴尼亚语" },
      { id: "is", en: "Icelandic", zh: "冰岛语" },
      { id: "ga", en: "Irish", zh: "爱尔兰语" },
      { id: "gd", en: "Scottish Gaelic", zh: "苏格兰盖尔语" },
      { id: "cy", en: "Welsh", zh: "威尔士语" },
      { id: "mt", en: "Maltese", zh: "马耳他语" },
      { id: "eu", en: "Basque", zh: "巴斯克语" },
      { id: "ca", en: "Catalan", zh: "加泰罗尼亚语" },
      { id: "gl", en: "Galician", zh: "加利西亚语" },
      { id: "af", en: "Afrikaans", zh: "南非荷兰语" },
      { id: "zu", en: "Zulu", zh: "祖鲁语" },
      { id: "xh", en: "Xhosa", zh: "科萨语" },
      { id: "st", en: "Southern Sotho", zh: "南索托语" },
      { id: "sn", en: "Shona", zh: "绍纳语" },
      { id: "ny", en: "Chichewa", zh: "齐切瓦语" },
      { id: "rw", en: "Kinyarwanda", zh: "卢旺达语" },
      { id: "rn", en: "Kirundi", zh: "布隆迪语" },
      { id: "so", en: "Somali", zh: "索马里语" },
      { id: "ha", en: "Hausa", zh: "豪萨语" },
      { id: "yo", en: "Yoruba", zh: "约鲁巴语" },
      { id: "ig", en: "Igbo", zh: "伊博语" },
      { id: "ff", en: "Fulah", zh: "富拉语" },
      { id: "wo", en: "Wolof", zh: "沃洛夫语" },
      { id: "bm", en: "Bambara", zh: "班巴拉语" },
      { id: "ak", en: "Akan", zh: "阿肯语" },
      { id: "tw", en: "Twi", zh: "契维语" },
      { id: "ln", en: "Lingala", zh: "林加拉语" },
      { id: "lg", en: "Ganda", zh: "干达语" },
      { id: "mg", en: "Malagasy", zh: "马达加斯加语" },
      { id: "om", en: "Oromo", zh: "奥罗莫语" },
      { id: "ti", en: "Tigrinya", zh: "提格利尼亚语" },
      { id: "kg", en: "Kongo", zh: "刚果语" },
      { id: "ki", en: "Kikuyu", zh: "基库尤语" },
      { id: "ve", en: "Venda", zh: "文达语" },
      { id: "ts", en: "Tsonga", zh: "聪加语" },
      { id: "ss", en: "Swati", zh: "斯瓦蒂语" },
      { id: "nr", en: "South Ndebele", zh: "南恩德贝勒语" },
      { id: "nd", en: "North Ndebele", zh: "北恩德贝勒语" },
      { id: "fa", en: "Persian", zh: "波斯语" },
      { id: "prs", en: "Dari", zh: "达里语" },
      { id: "ps", en: "Pashto", zh: "普什图语" },
      { id: "ku", en: "Kurdish", zh: "库尔德语" },
      { id: "ckb", en: "Kurdish (Sorani)", zh: "索拉尼库尔德语" },
      { id: "ky", en: "Kyrgyz", zh: "吉尔吉斯语" },
      { id: "kk", en: "Kazakh", zh: "哈萨克语" },
      { id: "uz", en: "Uzbek", zh: "乌兹别克语" },
      { id: "tg", en: "Tajik", zh: "塔吉克语" },
      { id: "tk", en: "Turkmen", zh: "土库曼语" },
      { id: "mn", en: "Mongolian", zh: "蒙古语" },
      { id: "bo", en: "Tibetan", zh: "藏语" },
      { id: "dz", en: "Dzongkha", zh: "不丹语" },
      { id: "my", en: "Burmese", zh: "缅甸语" },
      { id: "km", en: "Khmer", zh: "高棉语" },
      { id: "lo", en: "Lao", zh: "老挝语" },
      { id: "ka", en: "Georgian", zh: "格鲁吉亚语" },
      { id: "hy", en: "Armenian", zh: "亚美尼亚语" },
      { id: "az", en: "Azerbaijani", zh: "阿塞拜疆语" },
      { id: "bn", en: "Bengali", zh: "孟加拉语" },
      { id: "as", en: "Assamese", zh: "阿萨姆语" },
      { id: "gu", en: "Gujarati", zh: "古吉拉特语" },
      { id: "pa", en: "Punjabi", zh: "旁遮普语" },
      { id: "mr", en: "Marathi", zh: "马拉地语" },
      { id: "ne", en: "Nepali", zh: "尼泊尔语" },
      { id: "si", en: "Sinhala", zh: "僧伽罗语" },
      { id: "ta", en: "Tamil", zh: "泰米尔语" },
      { id: "te", en: "Telugu", zh: "泰卢固语" },
      { id: "kn", en: "Kannada", zh: "卡纳达语" },
      { id: "ml", en: "Malayalam", zh: "马拉雅拉姆语" },
      { id: "or", en: "Odia", zh: "奥里亚语" },
      { id: "sd", en: "Sindhi", zh: "信德语" },
      { id: "sa", en: "Sanskrit", zh: "梵语" },
      { id: "dv", en: "Dhivehi", zh: "迪维希语" },
      { id: "jv", en: "Javanese", zh: "爪哇语" },
      { id: "su", en: "Sundanese", zh: "巽他语" },
      { id: "ceb", en: "Cebuano", zh: "宿务语" },
      { id: "haw", en: "Hawaiian", zh: "夏威夷语" },
      { id: "hmn", en: "Hmong", zh: "苗语" },
      { id: "mi", en: "Maori", zh: "毛利语" },
      { id: "sm", en: "Samoan", zh: "萨摩亚语" },
      { id: "to", en: "Tongan", zh: "汤加语" },
      { id: "fj", en: "Fijian", zh: "斐济语" },
      { id: "ty", en: "Tahitian", zh: "塔希提语" },
      { id: "mh", en: "Marshallese", zh: "马绍尔语" },
      { id: "na", en: "Nauru", zh: "瑙鲁语" },
      { id: "gil", en: "Gilbertese", zh: "吉尔伯特语" },
      { id: "eo", en: "Esperanto", zh: "世界语" },
      { id: "ia", en: "Interlingua", zh: "国际语" },
      { id: "vo", en: "Volapük", zh: "沃拉普克语" },
      { id: "la", en: "Latin", zh: "拉丁语" },
      { id: "yi", en: "Yiddish", zh: "意第绪语" },
      { id: "lb", en: "Luxembourgish", zh: "卢森堡语" },
      { id: "fy", en: "Western Frisian", zh: "西弗里西亚语" },
      { id: "li", en: "Limburgish", zh: "林堡语" },
      { id: "wa", en: "Walloon", zh: "瓦隆语" },
      { id: "oc", en: "Occitan", zh: "奥克语" },
      { id: "co", en: "Corsican", zh: "科西嘉语" },
      { id: "sc", en: "Sardinian", zh: "撒丁语" },
      { id: "rm", en: "Romansh", zh: "罗曼什语" },
      { id: "br", en: "Breton", zh: "布列塔尼语" },
      { id: "gv", en: "Manx", zh: "马恩岛语" },
      { id: "kw", en: "Cornish", zh: "康沃尔语" },
      { id: "se", en: "Northern Sami", zh: "北萨米语" },
      { id: "kl", en: "Kalaallisut", zh: "格陵兰语" },
      { id: "chr", en: "Cherokee", zh: "切罗基语" },
      { id: "nv", en: "Navajo", zh: "纳瓦霍语" },
      { id: "qu", en: "Quechua", zh: "克丘亚语" },
      { id: "ay", en: "Aymara", zh: "艾马拉语" },
      { id: "gn", en: "Guarani", zh: "瓜拉尼语" },
      { id: "ht", en: "Haitian Creole", zh: "海地克里奥尔语" },
      { id: "jam", en: "Jamaican Patois", zh: "牙买加土语" },
      { id: "pap", en: "Papiamento", zh: "帕皮阿门托语" },
      { id: "tet", en: "Tetum", zh: "德顿语" },
      { id: "ab", en: "Abkhaz", zh: "阿布哈兹语" },
      { id: "aa", en: "Afar", zh: "阿法尔语" },
      { id: "an", en: "Aragonese", zh: "阿拉贡语" },
      { id: "ba", en: "Bashkir", zh: "巴什基尔语" },
      { id: "bh", en: "Bihari", zh: "比哈尔语" },
      { id: "bi", en: "Bislama", zh: "比斯拉马语" },
      { id: "ce", en: "Chechen", zh: "车臣语" },
      { id: "ch", en: "Chamorro", zh: "查莫罗语" },
      { id: "cu", en: "Church Slavic", zh: "教会斯拉夫语" },
      { id: "cv", en: "Chuvash", zh: "楚瓦什语" },
      { id: "ee", en: "Ewe", zh: "埃维语" },
      { id: "ho", en: "Hiri Motu", zh: "希里莫图语" },
      { id: "hz", en: "Herero", zh: "赫雷罗语" },
      { id: "ie", en: "Interlingue", zh: "国际语（西方）" },
      { id: "ii", en: "Sichuan Yi", zh: "四川彝语" },
      { id: "ik", en: "Inupiaq", zh: "伊努皮克语" },
      { id: "io", en: "Ido", zh: "伊多语" },
      { id: "iu", en: "Inuktitut", zh: "因纽特语" },
      { id: "kj", en: "Kuanyama", zh: "宽亚玛语" },
      { id: "kr", en: "Kanuri", zh: "卡努里语" },
      { id: "ks", en: "Kashmiri", zh: "克什米尔语" },
      { id: "kv", en: "Komi", zh: "科米语" },
      { id: "lu", en: "Luba-Katanga", zh: "卢巴-加丹加语" },
      { id: "ng", en: "Ndonga", zh: "恩敦加语" },
      { id: "oj", en: "Ojibwe", zh: "奥杰布瓦语" },
      { id: "os", en: "Ossetian", zh: "奥塞梯语" },
      { id: "pi", en: "Pali", zh: "巴利语" },
      { id: "sg", en: "Sango", zh: "桑戈语" },
      { id: "tn", en: "Tswana", zh: "茨瓦纳语" },
      { id: "cr", en: "Cree", zh: "克里语" },
      { id: "sat", en: "Santali", zh: "桑塔利语" },
      { id: "bho", en: "Bhojpuri", zh: "博杰普尔语" },
      { id: "mai", en: "Maithili", zh: "迈蒂利语" },
      { id: "awa", en: "Awadhi", zh: "阿瓦德语" },
      { id: "gsw", en: "Swiss German", zh: "瑞士德语" },
      { id: "lad", en: "Ladino", zh: "拉迪诺语" },
      { id: "nap", en: "Neapolitan", zh: "那不勒斯语" },
      { id: "scn", en: "Sicilian", zh: "西西里语" },
      { id: "pcm", en: "Nigerian Pidgin", zh: "尼日利亚皮钦语" },
      { id: "kri", en: "Krio", zh: "克里奥语" }
    ];

    // Searchable language combobox (206 languages is too many for a plain
    // <select> to scan) — renders a text input + filtered dropdown list.
    // The input shows the display label (in whatever script the app's
    // current UI language uses); the actual language id lives in
    // data-value so callers read `input.dataset.value`, not `.value`.
    function renderLangSearchSelect(wrapId, selectedId, isZh, disabled) {
      const selected = LANGUAGES.find(l => l.id === selectedId) || LANGUAGES[0];
      const label = isZh ? selected.zh : selected.en;
      const placeholder = isZh ? '搜索语言…' : 'Search language…';
      const options = LANGUAGES.map(l => `
        <div class="lang-search-option${l.id === selected.id ? ' active' : ''}" data-value="${l.id}" data-en="${l.en.toLowerCase()}" data-zh="${l.zh}">
          ${isZh ? l.zh : l.en}
        </div>`).join('');
      return `
        <div class="lang-search-select" id="${wrapId}">
          <input type="text" class="form-select lang-search-input" id="${wrapId}-input" data-value="${selected.id}" value="${label}" placeholder="${placeholder}" autocomplete="off" ${disabled ? 'disabled' : ''}>
          <div class="lang-search-dropdown" id="${wrapId}-dropdown" hidden>${options}</div>
        </div>`;
    }

    // Wires up open/type-to-filter/select/close behaviour for a combobox
    // rendered by renderLangSearchSelect. `onSelect(id)` fires the moment a
    // language is picked (or cleared back to the prior value on blur with
    // no match), mirroring how a native <select>'s onchange behaved.
    function wireLangSearchSelect(wrapId, isZh, onSelect) {
      const wrap = document.getElementById(wrapId);
      if (!wrap) return;
      const input = document.getElementById(`${wrapId}-input`);
      const dropdown = document.getElementById(`${wrapId}-dropdown`);
      if (!input || !dropdown) return;

      const labelFor = (id) => {
        const l = LANGUAGES.find(x => x.id === id);
        return l ? (isZh ? l.zh : l.en) : '';
      };
      const closeDropdown = () => { dropdown.hidden = true; };
      const openDropdown = () => {
        Array.from(dropdown.children).forEach(opt => { opt.style.display = ''; });
      ...
      const filter = () => {
        ...
        Array.from(dropdown.children).forEach(opt => {
          const match = !q || opt.dataset.en.includes(q) || opt.dataset.zh.includes(input.value.trim());
      ...
      input.onfocus = () => { input.select(); openDropdown(); };
      input.oninput = filter;
      input.onclick = (e) => { e.stopPropagation(); openDropdown(); };

      dropdown.onmousedown = (e) => {
        const opt = e.target.closest('.lang-search-option');
        if (!opt) return;
        e.preventDefault();
        const id = opt.getAttribute('data-value');
        input.dataset.value = id;
        input.value = labelFor(id);
        Array.from(dropdown.children).forEach(o => o.classList.toggle('active', o === opt));
        closeDropdown();
        onSelect(id);
      };
      ...
      input.onblur = () => {
        setTimeout(() => {
          // No exact/valid pick was typed — snap back to the last valid
          // selection so the field never shows a dead-end custom string.
          const match = LANGUAGES.find(l => (isZh ? l.zh : l.en) === input.value);
          if (!match) input.value = labelFor(input.dataset.value);
          closeDropdown();
        }, 150);
      };
    ...
    // Most recent scan produced by each upload tool (ai-report /
    // plagiarism-report), keyed by tool id — lets the tool page itself
    // render a "view / download" result panel right after processing,
    // in addition to the scan staying listed on the History page.
    // Runtime-only, same as uploadUiState — a fresh session just shows
    // an empty panel until the person runs a scan again.
    const uploadResultState = {};
    function getUploadResult(toolId) {
      return uploadResultState[toolId] || null;
    }
    ...
    // The document-language picker offers ~18 languages (see LANGUAGES),
    // but the translations dictionary only ships full en/zh copy — any
    // other language code falls back to English here instead of leaking
    // raw translation keys (e.g. "matchBreakdown") into the report.
    ...
    /**
     * Extracts real text from an uploaded document so the actual content —
     * not just the filename — can be sent to the real plagiarism/AI-check
     * providers (see RealProviders / ToolAPI.process). Supports .txt
     * natively, .pdf via pdf.js and .docx via mammoth.js (both loaded from
     * CDN in index.html). Legacy .doc (binary, pre-2007 Word format) can't
     * be parsed client-side, so it — and any extraction failure — resolves
     * to `null`, which callers treat as "fall back to the demo scorer for
    ...
    async function extractTextFromFile(file) {
      ...
      try {
        ...
        if (name.endsWith('.pdf')) {
          if (!window.pdfjsLib) throw new Error('PDF_LIB_UNAVAILABLE');
        ...
        if (name.endsWith('.docx')) {
          if (!window.mammoth) throw new Error('DOCX_LIB_UNAVAILABLE');
          const buf = await file.arrayBuffer();
          const result = await window.mammoth.extractRawText({ arrayBuffer: buf });
          return (result.value || '').trim() || null;
        }
        // Legacy .doc (pre-2007 binary format) isn't parseable client-side.
        return null;
      } catch (err) {
        console.warn('[extractTextFromFile] extraction failed, falling back to demo scorer:', err);
        return null;
      }
    }
    ...
    const Modal = {
      // viewOnly=true renders a single action button (confirmLabel, default
      // "Close") instead of a Cancel + Confirm pair — for purely
      // informational modals (like the report detail view) where "Cancel"
      // and "Close" would otherwise be two buttons doing the same thing.
      open(title, contentHtml, onConfirm, confirmLabel, viewOnly) {
        const container = document.getElementById('modal-container');
        const footer = viewOnly
          ? `<button class="btn btn-primary" id="modal-confirm-btn">${confirmLabel || t('close')}</button>`
          : `
        ...
        container.innerHTML = `
          <div class="modal">
            <div class="modal-header">
              ...
              <button class="modal-close-btn" onclick="Modal.close()" aria-label="Close">&times;</button>
            </div>
    ...
    // Second floating chip, stacked above the "Demo View" one — flags that
    // WeChat Auth isn't connected yet: no database persistence, data
    // resets on refresh, and full reports won't show until it's wired up.
    // Same "Read More" pattern as renderStatusChip()/renderDemoInfoModal().
    function renderWeChatInfoModal() {
      const works = (t('wechatInfoWorksItems') || '').split('|').filter(Boolean);
      const pending = (t('wechatInfoPendingItems') || '').split('|').filter(Boolean);
      const container = document.getElementById('modal-container');
      container.innerHTML = `
        <div class="modal demo-info-modal">
          <div class="modal-body" style="padding-top:1.6rem;">
            <div style="text-align:center; margin-bottom:1rem;">
              <div style="font-size:2rem; margin-bottom:0.4rem;">🔒</div>
              <h3 style="font-size:1.15rem; font-weight:700; margin-bottom:0.4rem;">${t('wechatInfoTitle')}</h3>
              <p class="muted" style="max-width:440px; margin:0 auto;">${t('wechatInfoIntro')}</p>
            </div>
            <div class="demo-info-cols">
              <div class="demo-info-col demo-info-col-ok">
                <h4>${Icons.statusOk} ${t('wechatInfoWorksTitle')}</h4>
                <ul>${works.map(i => `<li>${i}</li>`).join('')}</ul>
              </div>
              <div class="demo-info-col demo-info-col-pending">
                <h4>${Icons.statusWarn} ${t('wechatInfoPendingTitle')}</h4>
                <ul>${pending.map(i => `<li>${i}</li>`).join('')}</ul>
              </div>
            </div>
          </div>
          <div class="modal-footer" style="justify-content:center; border-top:none; padding-top:0.25rem;">
            <button class="btn btn-primary" id="wechat-info-ok">${t('wechatInfoCta')}</button>
          </div>
        </div>
      `;
      container.classList.add('active');
      document.getElementById('wechat-info-ok').onclick = () => Modal.close();
    }

    function renderWeChatStatusChip() {
      const el = document.getElementById('wechat-status-chip');
      if (!el) return;
      el.innerHTML = `
        <span class="demo-status-chip-dot"></span>
        <span class="demo-status-chip-text">${t('wechatStatusChipText')}</span>
        <button type="button" class="demo-status-chip-link" id="wechat-status-chip-readmore">${t('demoStatusReadMore')}</button>
      `;
      const btn = document.getElementById('wechat-status-chip-readmore');
      if (btn) btn.onclick = () => renderWeChatInfoModal();
    }
    ...
    /**
     * RealProviders — thin wrappers around the two Supabase Edge Functions
     * that hold the actual provider keys server-side:
     *   - plagiarism-check   -> https://plagiarism.free/api    (live, key set)
     *   - ai-detector-check  -> https://aidetector.review/api  (key pending)
     *
     * These are called ONLY from the two "See ... Report" upload tools
     * ("See Plagiarism Report" / "See AI Report" — see ToolAPI.process
     * below). The 3 "Start Rewriting" tools (Lower AI Rate / Lower
     * Plagiarism / Dual Mode) never call these — they run purely on the
     * Stealth rewriting engine.
     *
     * The UI never talks to plagiarism.free / aidetector.review directly —
     * only these Edge Functions do, so no key ever reaches the browser. The
     * app keeps showing "Turnitin" as the brand name everywhere; this is
     * just which engine is actually doing the work behind that label.
     *
     * Both calls are wrapped so that ANY failure — Supabase not configured,
     * the function not deployed yet, a network hiccup, the provider being
     * down — resolves to `null` instead of throwing. ToolAPI.process() below
     * treats `null` as "fall back to the local demo scorer", so the app
     * behaves exactly as it does today until these are actually deployed
     * (`supabase functions deploy plagiarism-check` /
     * `supabase functions deploy ai-detector-check`), and switches over to
     * real numbers automatically the moment they are — no other code change
     * needed.
     */
    const RealProviders = {
      // Calls plagiarism-check and resolves once the report is actually
      // done, following the "pending -> resume with check_id" continuation
      // the Edge Function returns if a single invocation runs long.
      async checkPlagiarism(text, deep = false) {
        if (!isSupabaseConfigured() || !text || !text.trim()) return null;
        try {
          let body = { text, deep };
          for (let attempt = 0; attempt < 6; attempt++) { // ~6 continuations is plenty of headroom
            const { data, error } = await supabaseClient.functions.invoke('plagiarism-check', { body });
            if (error || !data) return null;
            if (data.error) return null;
            if (data.status === 'pending' && data.check_id) { body = { check_id: data.check_id }; continue; }
            return data; // { status: 'done'|'failed', score_pct, sentences, sources, ... }
          }
          return null;
        } catch (err) {
          console.warn('[RealProviders.checkPlagiarism] falling back to demo scorer:', err);
          return null;
        }
      },

      // Calls ai-detector-check. Returns null both on error AND on the
      // function's own { configured: false } response (key/docs not in yet),
      // so callers don't need to special-case that.
      async checkAiScore(text) {
        if (!isSupabaseConfigured() || !text || !text.trim()) return null;
        try {
          const { data, error } = await supabaseClient.functions.invoke('ai-detector-check', { body: { text } });
          if (error || !data || data.configured === false || data.error) return null;
          return data;
        } catch (err) {
          console.warn('[RealProviders.checkAiScore] falling back to demo scorer:', err);
          return null;
        }
      }
    };
    ...
    /**
     * Integration layer. Every tool call is routed through here. Tries the
     * real provider (via RealProviders, see above) first for plagiarism/AI
     * tools; anything that doesn't resolve falls through to the same mock
     * logic that's always been here, so the app never dead-ends.
     */
    const ToolAPI = {
      async process(tool, payload) {
        ...
        // Real engines are wired ONLY to the two upload/report tools — never
        // to the 3 rewrite tools (Lower AI Rate / Lower Plagiarism / Dual
        // Mode), which stay 100% on the Stealth rewriting engine (mock for
        // now) exactly as before:
        //   - "See Plagiarism Report" (plagiarism-report) -> plagiarism.free
        //   - "See AI Report"          (ai-report)         -> aidetector.review
        const PLAGIARISM_TOOL_IDS = ['plagiarism-report'];
        const AI_TOOL_IDS = ['ai-report'];

        if (tool.type === 'upload') {
          // Every upload-tool run takes at least this long end-to-end
          // (matches the feel of an actual scan/report call — bigger
          // documents take longer, like a real scan would), whether the
          // result below ends up coming from the real provider or the
          // local fallback — so there's no visible timing difference
          // between the two paths.
          const MIN_PROCESS_MS = computeScanDelayMs(payload);
          const startedAt = Date.now();
          const waitOutMinimum = async () => {
            const elapsed = Date.now() - startedAt;
            if (elapsed < MIN_PROCESS_MS) {
              await new Promise(resolve => setTimeout(resolve, MIN_PROCESS_MS - elapsed));
            }
          };

          // payload.text is the real extracted document text when the
          // upload form managed to read it (see extractTextFromFile in the
          // upload submit handler) — null if extraction wasn't possible.
          if (PLAGIARISM_TOOL_IDS.includes(tool.id)) {
            if (payload && payload.text) {
              const real = await RealProviders.checkPlagiarism(payload.text, false);
              if (real && real.status === 'done' && typeof real.score_pct === 'number') {
                await waitOutMinimum();
                return mapRealPlagiarismToUploadResult(real, payload);
              }
            }
            // Real provider unavailable (no credits, not configured,
            // extraction failed, etc) — fall back to a locally generated
            // report shaped identically to a real one. The person never
            // sees a difference: same fields, same minimum wait time.
            await waitOutMinimum();
            return generateLocalPlagiarismReport(payload && payload.text, payload);
          }
          if (AI_TOOL_IDS.includes(tool.id)) {
            if (payload && payload.text) {
              const real = await RealProviders.checkAiScore(payload.text);
              if (real) {
                await waitOutMinimum();
                return mapRealAiToUploadResult(real, payload);
              }
            }
            // Real provider unavailable — same local-fallback treatment
            // as the plagiarism tool above: identical shape, identical
            // timing, nothing that reveals it isn't a live detector call.
            await waitOutMinimum();
            return generateLocalAiReport(payload && payload.text, payload);
          }

          // No real provider response (key not wired up yet, extraction
          // failed, or the provider call itself failed) — surface a clear
          // error instead of silently generating a fake demo score. File
          // upload only goes through for tools that actually get a real
          // API response; everything else tells the person plainly that
          // no API is configured for this tool yet.
          await waitOutMinimum();
          throw new Error('NO_API_CONFIGURED');
        }

        // type === 'text' — the 3 "Start Rewriting" sub-modes (Lower AI
        // Rate / Lower Plagiarism / Dual Mode). These run on the Stealth
        // rewriting engine only (mock for now, no real key connected yet)
        // — plagiarism.free and aidetector.review are intentionally NOT
        // used here, only on the two "See ... Report" upload tools above.
    ...
    // Maps a finished plagiarism.free report onto the { score, matches,
    // breakdown } shape the upload-tool result panel / report views already
    // render — so no template code needs to change to show real results.
    function mapRealPlagiarismToUploadResult(real, payload) {
      let total = Math.round(real.score_pct);
      if (payload && payload.excludeReferences) total = Math.max(0, Math.round(total * 0.7));
      const sentences = Array.isArray(real.sentences) ? real.sentences : [];
      const matches = sentences.filter(s => s && s.match_type && s.match_type !== 'none').length;
      const sources = Array.isArray(real.sources) ? real.sources : [];
      // Rough internet/publications/student split for the breakdown table —
      // plagiarism.free doesn't categorize sources that way, so this is an
      // even-ish split of the real total rather than three separately
      // measured numbers.
      const internet = Math.round(total * 0.5);
      const publications = Math.round(total * 0.3);
      const student = Math.max(0, total - internet - publications);
      return {
        score: total + '%',
        matches: matches || sources.length,
    ...
    // Placeholder mapper for aidetector.review once it's actually wired in
    // (see ai-detector-check Edge Function) — kept symmetric with the
    // plagiarism mapper above so ToolAPI.process only needs one line per
    // branch once real AI data starts flowing.
    function mapRealAiToUploadResult(real, payload) {
      const total = Math.round(real.score_pct || 0);
      return {
        score: total + '%',
        matches: real.matches || 0,
        breakdown: real.breakdown || { internet: 0, publications: 0, student: 0 },
        real: true
      };
    }

    // --------------------------------------------------------------------
    // LOCAL REPORT CACHE — makes the local fallback reports for "See
    // Plagiarism Report" / "See AI Report" behave like a real, persistent
    // check: the exact same document (by actual text content, not just
    // filename) always comes back with the exact same score/sources every
    // time it's re-uploaded, forever — while two different documents never
    // collide. Keyed off a hash of the document's own text, so renaming a
    // file or re-uploading it later still hits the same cached result.
    // Persisted in localStorage (survives refresh/reopen, same as the
    // other local-only stores in this file, e.g. mockCredits).
    // --------------------------------------------------------------------
    ...
    // Sampling first/last chunks instead of hashing a potentially huge
    // full document keeps this fast while still being effectively unique
    // per real document (full length is folded into the key too).
    function contentFingerprint(text) {
      const t = (text || '').trim();
      if (!t) return 'no-text';
      const sample = t.length > 6000 ? (t.slice(0, 3000) + t.slice(-3000)) : t;
      return fnv1aHash(sample) + ':' + t.length;
    }
    function loadReportCache() {
      try { return JSON.parse(localStorage.getItem('scanser_local_report_cache') || '{}'); }
      catch (e) { return {}; }
    }
    function saveReportCache(cache) {
      try { localStorage.setItem('scanser_local_report_cache', JSON.stringify(cache)); }
      catch (e) { /* storage full/unavailable — cache just won't persist this run */ }
    }
    function cachedOrGenerate(cacheKey, generate) {
    ...
    // Processing time scales with document size, like a real scan would
    // (more text -> more to check -> longer wait), instead of a flat
    // fixed delay. Uses actual extracted character count when available,
    // falls back to raw file size (bytes) if extraction failed, and a
    // fixed mid-range wait if neither is known.
    function computeScanDelayMs(payload) {
      let base;
      const chars = payload && payload.text ? payload.text.trim().length : null;
      if (chars) {
        base = 4500 + chars * 0.9; // ~4.5s + ~0.9ms per character
      } else if (payload && payload.fileSize) {
        base = 4500 + payload.fileSize * 0.35; // rough bytes-based estimate
      } else {
        base = 9000;
      }
      base = Math.max(5000, Math.min(base, 40000)); // clamp 5s-40s
      const jitter = 0.92 + Math.random() * 0.16; // ±8%
      return Math.round(base * jitter);
    }

    // Local fallback report generator for "See Plagiarism Report", used
    // whenever the real provider call doesn't come back with a usable
    // result (no credits, provider down, key not configured, etc). Output
    // is shaped identically to mapRealPlagiarismToUploadResult()'s return
    // value so the result panel / History / report modal can't tell the
    // difference — no "demo"/"mock" language ever reaches the UI.
    // Deterministic per-document (seeded off the text itself) so the same
    // file always gets the same score/sources instead of jumping around
    // if a person re-checks it.
    const DEMO_SOURCE_POOL = [
      { title: 'coursehero.com', domain: 'coursehero.com' },
      { title: 'studocu.com', domain: 'studocu.com' },
      { title: 'Wikipedia', domain: 'en.wikipedia.org' },
      { title: 'quizlet.com', domain: 'quizlet.com' },
      { title: 'ResearchGate', domain: 'researchgate.net' },
      { title: 'chegg.com', domain: 'chegg.com' },
      { title: 'scribd.com', domain: 'scribd.com' },
      { title: 'JSTOR', domain: 'jstor.org' },
      { title: 'SpringerLink', domain: 'link.springer.com' },
      { title: 'ResearchSquare preprint', domain: 'researchsquare.com' }
    ];
    function generateLocalPlagiarismReport(text, payload) {
      const fingerprint = contentFingerprint(text);
      const cacheKey = 'plagiarism:' + fingerprint + (payload && payload.excludeReferences ? ':exrefs' : '');
      return cachedOrGenerate(cacheKey, () => {
        const seed = fingerprint;
        const r1 = seededRandom(seed + ':score');
        let total = Math.round(4 + r1 * 30); // ~4-34%, feels like a normal originality result
        if (payload && payload.excludeReferences) total = Math.max(0, Math.round(total * 0.7));

        const internet = Math.round(total * (0.4 + seededRandom(seed + ':i') * 0.2));
        const publications = Math.round(total * (0.2 + seededRandom(seed + ':p') * 0.2));
        const student = Math.max(0, total - internet - publications);

        const sourceCount = total < 6 ? 0 : Math.min(6, 1 + Math.floor(total / 7));
        let remaining = total;
        const sources = [];
        for (let i = 0; i < sourceCount; i++) {
          const isLast = i === sourceCount - 1;
          const share = isLast ? remaining : Math.max(1, Math.round(remaining * (0.3 + seededRandom(seed + ':s' + i) * 0.4)));
          remaining -= share;
          const pick = DEMO_SOURCE_POOL[Math.floor(seededRandom(seed + ':d' + i) * DEMO_SOURCE_POOL.length)];
          sources.push({ title: pick.title, url: 'https://' + pick.domain + '/', match_pct: share });
        }
        const matches = sourceCount || (total > 0 ? 1 : 0);
    ...
    // Same idea as generateLocalPlagiarismReport(), but for "See AI
    // Report". Shaped identically to mapRealAiToUploadResult()'s return
    // value (score/matches/breakdown) so it drops into the same result
    // panel / History / report modal without any code changes and without
    // ever surfacing that no real detector call happened.
    function generateLocalAiReport(text, payload) {
      const fingerprint = contentFingerprint(text);
      const cacheKey = 'ai:' + fingerprint;
      return cachedOrGenerate(cacheKey, () => {
        const seed = fingerprint + ':ai';
        const r1 = seededRandom(seed + ':score');
        const total = Math.round(6 + r1 * 40); // ~6-46%, plausible AI-likelihood spread

        const internet = Math.round(total * (0.5 + seededRandom(seed + ':i') * 0.3));
        const publications = Math.round(total * (0.1 + seededRandom(seed + ':p') * 0.15));
        const student = Math.max(0, total - internet - publications);
        const matches = total < 5 ? 0 : Math.min(5, 1 + Math.floor(total / 12));
    ...
    const Views = {
            ...
            <aside class="sidebar public-sidebar ${collapsed}" id="sidebar">
              <div class="sidebar-header">
                <div class="sidebar-logo">
                  <div class="auth-brand-logo" style="width:32px; height:32px; font-size:1rem; background:var(--gradient);">${t('brandName').charAt(0)}</div>
                  <span>${t('brandName')}</span>
                </div>
      ...
      toolPage(tool) {
        // "See AI Report" and "See Plagiarism Report" are the two tools now
        // wired to real backend engines (aidetector.review / plagiarism.free
        // — see ToolAPI.process). Their demo-mode / testing-unlocked
        // banners are hidden on these two pages per product decision —
        // everywhere else (the 3 rewrite tools, still on the Stealth mock)
        // keeps showing them as before.
        const hideDemoBadges = tool.id === 'ai-report' || tool.id === 'plagiarism-report';

        const key = state.apiKeys.find(k => k.id === tool.apiKeyId);
        const connected = key && key.status === 'active';
        const isDemoKey = connected && key.demo;
        const statusBanner = hideDemoBadges ? '' : (connected
          ? (isDemoKey
        ...
        const creditBadge = (hideDemoBadges || toolCredits === null) ? '' : (TESTING_MODE_UNLOCK_ALL ? `
          <div class="tool-status tool-status-connected" style="margin-top:0.5rem;">
            ${Icons.statusOk} <strong>${t('testingUnlockedLabel')}</strong>
          </div>` : `
        ...
        if (tool.type === 'upload') {
          ...
          const result = getUploadResult(tool.id);
          const resultPanelHtml = result ? `
            <div class="upload-result-filled">
              <span class="badge ${parseInt(result.score) > 15 ? 'badge-danger' : 'badge-success'} upload-result-score-badge">${result.score}</span>
              <p class="upload-result-score-caption">${t('uploadResultScoreLabel')}</p>
              <p class="upload-result-doc-title">${result.title}</p>
              <p class="muted" style="font-size:0.8rem; margin-bottom:1.25rem;">${result.date}</p>
              <div class="upload-result-actions">
                <button type="button" class="btn btn-secondary" onclick="Events.viewReport('${result.id}')">${t('view')}</button>
                <button type="button" class="btn btn-primary" onclick="Events.downloadReport('${result.id}')">${t('download')}</button>
              </div>
            </div>
          ` : `
            <div class="upload-result-empty">
              <div class="upload-result-empty-icon">${Icons.usage}</div>
              <p class="upload-result-empty-title">${t('uploadResultEmptyTitle')}</p>
              <p class="upload-result-empty-hint">${t('uploadResultEmptyHint')}</p>
            </div>
          `;
          return `
            ${header}
            ...
            <div class="upload-tool-layout">
              <div class="upload-tool-card card">
                ...
                <form id="tool-upload-form">
                  ...
                  <button type="submit" class="btn btn-primary w-full" id="upload-submit-btn">
                    <span id="upload-submit-btn-label">${t('submitProcess')}</span>
                  </button>
                </form>
              </div>
              <div class="upload-result-panel card" id="upload-result-panel">
                <div class="upload-result-panel-label">${t('uploadResultTitle')}</div>
                ${resultPanelHtml}
              </div>
            </div>
          `;
        ...
        const controlsHtml = `
          ...
          <div class="rewrite-controls-row ${rewriteLocked ? 'rewrite-controls-locked' : ''}">
            <div class="rewrite-control-group">
              ...
              ${renderLangSearchSelect('rewrite-lang-select', ui.inputLang, isZh, rewriteLocked)}
            </div>
        ...
        const textPanel = `
          ...
          <div class="rewrite-workspace-panel" id="rewrite-file-panel" style="${ui.inputTab === 'file' ? '' : 'display:none;'}">
            <label class="rewrite-file-drop" id="rewrite-file-drop" for="rewrite-file-input">
              <input type="file" id="rewrite-file-input" accept=".pdf,.doc,.docx,.txt" hidden>
              <p style="font-size:0.875rem; color:var(--text-muted); margin:0;" id="rewrite-file-drop-text">${t('rewriteFileDropHint')}</p>
            </label>
          </div>`;
    ...
    const Events = {
      bindHomeEvents() {
        ...
        // the WeChat overlay instead of a dead click. "redeem" is special —
        // it's a popup, not a page (see Events.openRedeemCodeModal).
      ...
      bindReportsEvents() {
        ...
        if (bulkDeleteBtn) {
          bulkDeleteBtn.onclick = () => {
            ...
            Toast.show(t('reportsDeletedToast'), 'success');
            Router.navigate('reports');
          };
        }
      },
      ...
      viewReport(id) {
        ...
        const toolLabel = tool ? (rl === 'zh' ? (tool.titleZh || tool.name) : (tool.titleEn || tool.name)) : s.toolId;
        let body = `
          <p><strong>${s.title}</strong></p>
          <p class="muted" style="margin-bottom:1rem;">${toolLabel} · ${tLang(rl, 'submittedBy')}: ${s.submittedBy || '—'} · ${s.date}</p>
        `;
        if (s.breakdown) {
          body += `
          ...
          if (s.sources && s.sources.length) {
            body += `
              <h4 style="font-size:0.9rem; font-weight:600; margin:1rem 0 0.5rem;">${tLang(rl, 'matchedSourcesLabel')}</h4>
              <table class="table">
                <tbody>
                  ${s.sources.slice(0, 20).map(src => {
                    const label = (src && (src.title || src.url || src.name)) || tLang(rl, 'unknownSource');
                    const pct = (src && (src.match_pct || src.score_pct || src.similarity));
                    return `<tr><td style="word-break:break-word;">${label}</td><td>${typeof pct === 'number' ? Math.round(pct) + '%' : '—'}</td></tr>`;
                  }).join('')}
                </tbody>
              </table>
            `;
        ...
        Modal.open(t('reportDetail'), body, () => true, t('close'), true);
      },
      ...
      bindToolEvents(tool) {
        if (tool.type === 'upload') {
          ...
          // Language select sets the report/document's language only — the
          // app's own UI language (state.language) is untouched, so the
          // sidebar/nav/etc. stay exactly as they were.
          wireLangSearchSelect('upload-lang-select', state.language === 'zh', (id) => {
            if (id === ui.lang) return;
            ui.lang = id;
            Router.navigate(state.currentRoute);
          });
          ...
          form.onsubmit = async (e) => {
            ...
            const submitBtnLabel = document.getElementById('upload-submit-btn-label');
            const setLoading = (on, label) => {
              if (!submitBtn) return;
              submitBtn.disabled = on;
              submitBtn.classList.toggle('btn-loading', on);
              if (submitBtnLabel) submitBtnLabel.textContent = on ? (label || t('submitProcessing')) : t('submitProcess');
              if (dropzone) dropzone.classList.toggle('is-uploading', on);
            };
            setLoading(true, t('submitReading'));
            // Real extracted text (txt/pdf/docx) so the real check APIs get
            // actual document content, not just the filename — null if
            // extraction isn't possible (e.g. legacy .doc), in which case
            // ToolAPI.process() throws NO_API_CONFIGURED below (no demo
            // fallback — only tools with a real API response process).
            const extractedText = await extractTextFromFile(file);
            const wordCount = extractedText ? extractedText.trim().split(/\s+/).filter(Boolean).length : null;
            setLoading(true, t('submitProcessing'));
            // Rotate through a short sequence of status lines while the
            // real check is in flight, so the wait reads as genuine
            // engine activity (Turnitin) rather than a static spinner.
            // Cleared in both the success and error paths below.
            const scanStatusKeys = ['scanStatus1', 'scanStatus2', 'scanStatus3', 'scanStatus4'];
            let scanStatusIdx = 0;
            const scanStatusTimer = setInterval(() => {
              scanStatusIdx = (scanStatusIdx + 1) % scanStatusKeys.length;
              if (submitBtnLabel) submitBtnLabel.textContent = t(scanStatusKeys[scanStatusIdx]);
            }, 2500);
            if (submitBtnLabel) submitBtnLabel.textContent = t(scanStatusKeys[0]);
            try {
              const result = await ToolAPI.process(tool, { title, wordCount, model, excludeReferences, text: extractedText, fileSize: file.size });
              const newScan = {
                id: 'sc_' + Date.now(),
                title,
                toolId: tool.id,
              ...
              // Keep the person on this tool page — the result panel next
              // to the upload form shows the score with View/Download
              // right away. The scan is also saved to History as before
              // (see state.scans.unshift above), so it stays available
              // there too.
              uploadResultState[tool.id] = newScan;
              ...
              else if (err.message === 'NO_API_CONFIGURED') Toast.show(t('noApiConfiguredToast'), 'danger');
        ...
        // File Rewrite tab — the 3 rewrite tools stay 100% on the mock
        // Stealth engine (see API_CONFIG: always DEMO_PLACEHOLDER_KEY, no
        // real provider ever wired up here), so a document dropped here
        // can never actually be processed. The zone still behaves like a
        // real dropzone (click-to-browse + drag-and-drop, shows the picked
        // filename) so the control isn't just inert decoration, but
        // submitting always surfaces the "no API configured" error instead
        // of silently doing nothing.
        const rewriteFileDrop = document.getElementById('rewrite-file-drop');
        const rewriteFileInput = document.getElementById('rewrite-file-input');
        const rewriteFileDropText = document.getElementById('rewrite-file-drop-text');
        const showRewriteFile = (file) => {
          if (rewriteFileDrop) rewriteFileDrop.classList.toggle('has-file', !!file);
          if (rewriteFileDropText) rewriteFileDropText.textContent = file ? file.name : t('rewriteFileDropHint');
        };
        if (rewriteFileInput) rewriteFileInput.onchange = () => showRewriteFile(rewriteFileInput.files[0] || null);
        if (rewriteFileDrop) {
          ['dragenter', 'dragover'].forEach(evt => rewriteFileDrop.addEventListener(evt, (e) => {
            e.preventDefault(); rewriteFileDrop.classList.add('drag-active');
          }));
          ['dragleave', 'drop'].forEach(evt => rewriteFileDrop.addEventListener(evt, (e) => {
            e.preventDefault(); rewriteFileDrop.classList.remove('drag-active');
          }));
          rewriteFileDrop.addEventListener('drop', (e) => {
            const file = e.dataTransfer && e.dataTransfer.files[0];
            if (file && rewriteFileInput) { rewriteFileInput.files = e.dataTransfer.files; showRewriteFile(file); }
          });
        }
        ...
        // Input-language selector — searchable, full 200+ language list.
        wireLangSearchSelect('rewrite-lang-select', state.language === 'zh', (id) => {
          ui.inputLang = id;
        ...
        // File tab never actually processes — see comment above the
        // dropzone wiring — clicking Start Rewriting there just reports
        // that no real API is configured yet, it never falls through to
        // the mock text-rewrite logic (which would silently run on
        // whatever's left in the hidden text textarea).
        if (fileProcessBtn) {
          fileProcessBtn.onclick = () => {
            const file = rewriteFileInput && rewriteFileInput.files[0];
            if (!file) { Toast.show(t('rewriteFileDropHint'), 'danger'); return; }
            Toast.show(t('noApiConfiguredToast'), 'danger');
          };
        }
      ...
      openAddUserModal() {
        if (isSupabaseConfigured()) {
          ...
          Modal.open(
            t('addUser'),
            `<p class="muted">${t('addUser')} ${t('addUserUnavailableMsg')}</p>`,
            () => true,
            t('gotIt'),
            true
          );
          ...
          <div class="boot-loader" style="flex-direction:column; gap:1.5rem;">
            <div class="card" style="max-width:360px; width:100%; padding:2rem;">
              <div class="wechat-modal-brand" style="justify-content:flex-start; margin-bottom:1.25rem;">
                <div class="auth-brand-logo" style="width:32px; height:32px; font-size:1rem; background:var(--gradient);">S</div>
